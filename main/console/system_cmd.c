...
static int heap_size(int argc, char **argv)
{
    ...
    console_printf("min heap size: %" PRIu32 "\n", heap_size);
    return 0;
}
...
static int log_level(int argc, char **argv)
{
    ...
    {
        console_printf("Invalid log level '%s', choose from none|error|warn|info|debug|verbose\n", level_str);
        return 1;
    }
    if (level > CONFIG_LOG_MAXIMUM_LEVEL)
    {
        console_printf("Can't set log level to %s, max level limited in menuconfig to %s. "
